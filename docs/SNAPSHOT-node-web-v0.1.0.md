...
> **Nota:** este documento descreve a baseline **congelada** em `stable/node-web-v0.1.0`. A branch **`main`** evoluiu desde então (WebView2 nativo, snapshot JSON, botão **Abrir**, startup paralelo, etc.) — use `README.md` e `docs/ARCHITECTURE.md` para o estado atual. Código atual: `launcher/`, `web/`, `server/`.
...
## Evolução posterior (`main`, após este snapshot)
...
- **snapshot JSON na impressão** (`budget-snapshot.js` + `POST /api/pdf`): `.json` ao lado do `.pdf` em `output/`, `schemaVersion: 1`; nome `{paciente} - {cirurgia} {data} {hora}`; exportação **no servidor** (`pdf-export.js`); impressão só após `await exportPdfDocument()`;
- **importação de snapshot** (botão **Abrir**): `POST /api/open-snapshot`; seletor via `scripts/open-snapshot-dialog.py` (**pywebview**/WebView2, HiDPI); `lastSnapshotDir` em `data/settings.json`; libera porta 3000 ao relançar;
...
- **espelho SQLite** (`export/orcamentos.sqlite`): na abertura do servidor e após imprimir na stack Node, consolida todos os `output/*.json` (`server/export-sqlite.js`; ver `docs/export-sqlite.md`). Os JSON em `output/` continuam canônicos.
